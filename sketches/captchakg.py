...
train_labels = pd.read_json(TRAIN_DATASET_LABELS + os.path.sep + "train_labels.json", orient="index")
train_labels.columns =['label']
test_labels = pd.read_json(TEST_DATASET_LABELS + os.path.sep + "test_labels.json", orient="index")
...
for image in os.listdir(DIR):
    ...
    lkey =image.split(".")[0]
    print(lkey)
    labels.append(train_labels.index[int(lkey)])
...
def encode_targets():
  ...
  image_files = os.listdir(TRAIN_DATASET_PATH) + os.listdir(TEST_DATASET_PATH)
  
#   targets_orig = [x.split("/")[-1].split(".")[0] for x in image_files]
#   targets_flat = [c for clist in targets for c in clist] # squeeze
  targets_orig = list(train_labels["label"].values) + list(test_labels["label"].values)
